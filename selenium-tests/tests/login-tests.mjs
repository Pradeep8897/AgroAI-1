...
async function runLoginTests() {
  ...
  try {
    ...
    await driver.wait(until.elementLocated(By.id('login-submit-btn')), 10000)
...
async function testInvalidCredentials(driver) {
  ...
  const errorText = await getVisibleErrorText(driver)
  const pass = errorText && /invalid credentials|login failed|cannot connect/i.test(errorText)
...
async function testGoogleSignInButton(driver) {
  ...
  const googleButtons = await driver.findElements(By.xpath("//button[contains(normalize-space(.), 'Google') or contains(normalize-space(.), 'Continue with Google') or contains(normalize-space(.), 'Sign in with Google')]"))
